...
exports['match'] = {
  ...
  'basic matching': function(test) {
    ...
    test.deepEqual(globule.match('*.js', ['foo.js']), ['foo.js'], 'should match correctly.');
    test.deepEqual(globule.match('*.js', ['foo.js', 'bar.scss']), ['foo.js'], 'should match correctly.');
    test.deepEqual(globule.match(['*.js', '*.scss'], 'foo.js'), ['foo.js'], 'should match correctly.');
    test.deepEqual(globule.match(['*.js', '*.scss'], ['foo.js']), ['foo.js'], 'should match correctly.');
    test.deepEqual(globule.match(['*.js', '*.scss'], ['foo.js', 'bar.scss']), ['foo.js', 'bar.scss'], 'should match correctly.');
    test.done();
  },
  'no matches': function(test) {
    test.expect(2);
    test.deepEqual(globule.match('*.js', 'foo.scss'), [], 'should fail to match.');
    test.deepEqual(globule.match('*.js', ['foo.scss', 'bar.scss']), [], 'should fail to match.');
    test.done();
  },
  ...
  'flatten': function(test) {
    test.expect(1);
    test.deepEqual(globule.match([['*.js', '*.scss'], ['*.*', '*.js']], ['foo.js', 'bar.scss']),
      ['foo.js', 'bar.scss'],
...
exports['isMatch'] = {
  'basic matching': function(test) {
    ...
    test.ok(globule.isMatch('*.js', ['foo.js']), 'should match correctly.');
    test.ok(globule.isMatch('*.js', ['foo.js', 'bar.scss']), 'should match correctly.');
    test.ok(globule.isMatch(['*.js', '*.scss'], 'foo.js'), 'should match correctly.');
    test.ok(globule.isMatch(['*.js', '*.scss'], ['foo.js']), 'should match correctly.');
    test.ok(globule.isMatch(['*.js', '*.scss'], ['foo.js', 'bar.scss']), 'should match correctly.');
    test.done();
  },
  'no matches': function(test) {
    test.expect(6);
    test.ok(!globule.isMatch('*.js', 'foo.scss'), 'should fail to match.');
    test.ok(!globule.isMatch('*.js', ['foo.scss', 'bar.scss']), 'should fail to match.');
    test.ok(!globule.isMatch(null, 'foo.scss'), 'should fail to match.');
    test.ok(!globule.isMatch('*.js', null), 'should fail to match.');
    test.ok(!globule.isMatch([], 'foo.scss'), 'should fail to match.');
...
exports['find'] = {
  ...
  'basic matching': function(test) {
    ...
    test.deepEqual(globule.find('**/*.js', '**/*.scss'),
      ['js/bar.js', 'js/foo.js', 'scss/baz.scss', 'scss/qux.scss'],
      'multiple pattern arguments should match.');
    test.deepEqual(globule.find(['**/*.js', '**/*.scss']),
      ['js/bar.js', 'js/foo.js', 'scss/baz.scss', 'scss/qux.scss'],
      'array of patterns should match.');
    test.deepEqual(globule.find([['**/*.js'], [['**/*.scss', 'js/*.js']]]),
      ['js/bar.js', 'js/foo.js', 'scss/baz.scss', 'scss/qux.scss'],
  ...
  'unique': function(test) {
    ...
    test.deepEqual(globule.find('**/*.js', '**/*.scss', 'js/*.js'), ['js/bar.js', 'js/foo.js',
      'scss/baz.scss', 'scss/qux.scss'],
      'file list should be uniqed.');
  ...
  'file order': function(test) {
    test.expect(5);
    var actual = globule.find('**/*.{js,scss}');
    var expected = ['scss/baz.scss', 'scss/qux.scss', 'js/bar.js', 'js/foo.js'];
    test.deepEqual(actual, expected, 'should select 4 files in this order, by default.');

    actual = globule.find('js/foo.js', 'js/bar.js', '**/*.{js,scss}');
    expected = ['js/foo.js', 'js/bar.js', 'scss/baz.scss', 'scss/qux.scss'];
    test.deepEqual(actual, expected, 'specifically-specified-up-front file order should be maintained.');

    actual = globule.find('js/bar.js', 'js/foo.js', '**/*.{js,scss}');
    expected = ['js/bar.js', 'js/foo.js', 'scss/baz.scss', 'scss/qux.scss'];
    test.deepEqual(actual, expected, 'specifically-specified-up-front file order should be maintained.');

    actual = globule.find('**/*.{js,scss}', '!scss/qux.scss', 'scss/qux.scss');
    expected = ['scss/baz.scss', 'js/bar.js', 'js/foo.js', 'scss/qux.scss'];
    test.deepEqual(actual, expected, 'if a file is excluded and then re-added, it should be added at the end.');

    actual = globule.find('js/foo.js', '**/*.{js,scss}', '!scss/qux.scss', 'scss/qux.scss');
    expected = ['js/foo.js', 'scss/baz.scss', 'js/bar.js', 'scss/qux.scss'];
  ...
  'exclusion': function(test) {
    ...
    test.deepEqual(globule.find(['!js/foo.js', '**/*.js']),
      ['js/bar.js', 'js/foo.js'],
      'inclusion / exclusion order matters');
    test.deepEqual(globule.find(['**/*.js', '**/*.scss', '!js/bar.js', '!scss/baz.scss']),
      ['js/foo.js','scss/qux.scss'],
      'multiple exclusions should be removed from the set');
    test.deepEqual(globule.find(['**/*.js', '**/*.scss', '!**/*.scss']),
      ['js/bar.js', 'js/foo.js'],
      'excluded wildcards should be removed from the matched set');
    test.deepEqual(globule.find(['js/bar.js', 'js/foo.js', 'scss/baz.scss', 'scss/qux.scss', '!**/b*.*']),
      ['js/foo.js', 'scss/qux.scss'],
      'different pattern for exclusion should still work');
    test.deepEqual(globule.find(['js/bar.js', '!**/b*.*', 'js/foo.js', 'scss/baz.scss', 'scss/qux.scss']),
      ['js/foo.js', 'scss/baz.scss', 'scss/qux.scss'],
      'inclusion / exclusion order matters');
    test.done();
  },
  ...
  'options.filter': function(test) {
    ...
    test.deepEqual(globule.find('js', 'scss', {filter: 'isFile'}), [], 'should fail to match.');
  ...
  'options.matchBase': function(test) {
    ...
    test.deepEqual(globule.find('*.js', '*.scss', {matchBase: true}),
      ['js/bar.js', 'js/foo.js', 'scss/baz.scss', 'scss/qux.scss'],
      'matchBase option should be passed through to minimatch.');
    test.done();
  },
